...
class OtpView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = R.attr.otpViewStyle) : AppCompatEditText(context, attrs, defStyleAttr) {
    ...
    private var defAddAnimator: ValueAnimator? = null
    ...
    private fun setupAnimator() {
        defAddAnimator = ValueAnimator.ofFloat(0.5f, 1f)
        defAddAnimator!!.duration = 150
        defAddAnimator!!.interpolator = DecelerateInterpolator()
        defAddAnimator!!.addUpdateListener(AnimatorUpdateListener { animation ->
    ...
    override fun onTextChanged(text: CharSequence, start: Int, lengthBefore: Int, lengthAfter: Int) {
        ...
        if (isAnimationEnable) {
            ...
            if (isAdd && defAddAnimator != null) {
                defAddAnimator!!.end()
                defAddAnimator!!.start()
            }
        }
    }
    ...
    private fun drawOtpView(canvas: Canvas) {
        ...
        for (i in 0 until otpViewItemCount) {
            ...
            paint.color = itemState?.let { getLineColorForState(i) } ?: currentLineColor
            updateItemRectF(i)
            updateCenterPoint()
            canvas.save()
    ...
    private fun getLineColorForState(vararg states: Int): Int {
        return if (lineColors != null) lineColors!!.getColorForState(states,
                currentLineColor) else currentLineColor
    }
